...
from app.models import MatchupResponse, PlayerCardResponse, Window
...
class SnapshotStore:
    ...
    def get_latest_player_card(self, player_id: int) -> PlayerCardResponse | None:
        if self._backend.startswith("sqlite"):
            row = self._sqlite_get_latest_player_card_row(player_id=player_id)
        else:
            row = self._postgres_get_latest_player_card_row(player_id=player_id)
    ...
    def _sqlite_get_latest_player_card_row(self, player_id: int) -> tuple[Any, ...] | None:
        with self._lock:
            with self._sqlite_connect() as conn:
                return conn.execute(
                    """
    ...
    def _postgres_get_latest_player_card_row(self, player_id: int) -> Any | None:
        with self._lock:
            with self._postgres_connect() as conn:
                with conn.cursor() as cursor:
                    cursor.execute(
